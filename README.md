...
・**カフェモンスター戦闘（フェーズ0）** - プレイヤー／モンスターの戦闘ステータスと永続的な戦闘状態をMongoDBで管理します。現時点では開発者が接続を確認するための開始・終了のみで、攻撃と報酬はフェーズ1で追加します。
...
- 開発環境でのみ、`DEV_BATTLE_USER_IDS` に登録したDiscordユーザーIDは `！戦闘`（`!battle`）を使えます。戦闘カードはチャンネルに表示されますが、操作ボタンは開始本人以外にはephemeralで拒否されます。メッセージ型コマンドはDiscord仕様上ephemeralにできません。
- 戦闘状態は `battles` コレクションに保存され、起動時および5分ごとの掃除でタイムアウトします。フェーズ0の「戦闘を終了」ボタンは原子的なロックで連打を防ぎます。
