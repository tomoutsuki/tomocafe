...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('daily')
        .setDescription('【デイリー】毎日コーヒー豆をゲット！24時間に1回受け取れます。'),
        ...
        try {
            ...
            if (!user) {
                ...
                await interaction.reply({ 
                    content: `あなたはまだ登録していないようです！ **『/register』** コマンドを使ってみてください！`,
                    ephemeral: true 
                });
                return;
            }
            ...
            if (user.last_daily_claim) {
                ...
                if (hours < 24) {
                    ...
                    await interaction.reply({ 
                        content: `⏳ まだ受け取れません！あと **${remaining}時間** 待ってね！`,
                        ephemeral: true 
                    });
                    return;
                }
            ...
            await interaction.reply({ 
                content: `あなたは ˚﹢.${config.CURRENCY_EMOJI}×${beans} .˚﹢ のデイリーボーナスを受け取りました！✨` 
            });

        } catch (err) {
            console.error("デイリー実行時のエラー:", err);
            await interaction.reply({ 
